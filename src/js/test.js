...
export const addMainCards = (array, textForCardArr, mode, textForTranslateArr) =>{
    ...
    array.forEach(element => {
        ...
        if (pageStatus.category != 'Main Page'){
            let textForTranslate = textForTranslateArr[index];
...
const changeText = (child) =>{
    child.forEach(element => {
        if (element.classList.contains('train-text')){
            if (element.classList.contains('text-hidden')){
...
const reverseCard = (elementGrandParent, parent) => {
    elementGrandParent.classList.add('run-animation');
    changeText(parent.childNodes);
    function deleteAnimation (){
        elementGrandParent.classList.remove('run-animation');
      } 
      setTimeout(deleteAnimation, 1000);
}
//play audio for train
const playAudioForTrain = (str) =>{
    let audioTrain = new Audio();
    audioTrain.src = `/src/assets/mp3/${str}.mp3`;
    audioTrain.autoplay = true; 
}

mainImages.addEventListener('click', () =>{
    let elementGrandParent = event.target.parentElement.parentElement;
    if (event.target.parentElement.classList.contains('train-card') || event.target.classList.contains('train-card') || elementGrandParent.classList.contains('train-card')){
        if (event.target.classList.contains('button_reverse')){
            reverseCard(elementGrandParent, event.target.parentElement);
        }
        else {
